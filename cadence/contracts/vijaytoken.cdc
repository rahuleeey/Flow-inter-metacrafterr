...
// Vijaytoken contract: Implements fungible token on Flow blockchain
pub contract vijaytoken: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @vijaytoken.Vault
        ...
        // Destroy the vault
        destroy() {
            vijaytoken.totalSupply = vijaytoken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            vijaytoken.totalSupply = vijaytoken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&vijaytoken.Minter>(/public/Minter, target: /storage/MinterStorage)
