...
pub contract SwapToken {
    ...
    pub fun swapTokens(signer: AuthAccount, swapAmount: UFix64) {
        ...
        // Borrow vijaytoken and FlowToken vaults from the signer's storage
        let vijaytokenVault = signer.borrow<&vijaytoken.Vault>(from: /storage/VaultStorage)
            ?? panic("Could not borrow vijaytoken Vault from signer")
        ...
        let minterRef = signer.getCapability<&vijaytoken.Minter>(/public/Minter).borrow()
        ...
        vijaytokenVault.deposit(from: <-newVstokenVault)
